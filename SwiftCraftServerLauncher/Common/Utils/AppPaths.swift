...
enum AppPaths {
    ...
    static var serverRootDirectory: URL {
        let customPath = GeneralSettingsManager.shared.launcherWorkingDirectory
        let workingDirectory = customPath.isEmpty ? launcherSupportDirectory.path : customPath

        let baseURL = URL(fileURLWithPath: workingDirectory, isDirectory: true)
        return baseURL.appendingPathComponent(AppConstants.DirectoryNames.servers, isDirectory: true)
    }

    static var remoteNodesRootDirectory: URL {
        launcherSupportDirectory.appendingPathComponent("remote_nodes", isDirectory: true)
    }

    static func remoteNodeDirectory(nodeId: String) -> URL {
        cachedURL(key: "remoteNodeDirectory:\(nodeId)") {
            remoteNodesRootDirectory.appendingPathComponent(nodeId, isDirectory: true)
        }
    }

    static func remoteNodeServersDirectory(nodeId: String) -> URL {
        cachedURL(key: "remoteNodeServersDirectory:\(nodeId)") {
            remoteNodeDirectory(nodeId: nodeId).appendingPathComponent(AppConstants.DirectoryNames.servers, isDirectory: true)
        }
    }

    static func remoteNodeDataDirectory(nodeId: String) -> URL {
        cachedURL(key: "remoteNodeDataDirectory:\(nodeId)") {
            remoteNodeDirectory(nodeId: nodeId).appendingPathComponent(AppConstants.DirectoryNames.data, isDirectory: true)
        }
    }

    static func remoteNodeLogsDirectory(nodeId: String) -> URL {
        cachedURL(key: "remoteNodeLogsDirectory:\(nodeId)") {
            remoteNodeDirectory(nodeId: nodeId).appendingPathComponent(AppConstants.DirectoryNames.logs, isDirectory: true)
        }
    }

    static func serverDirectory(serverName: String) -> URL {
        cachedURL(key: "serverDirectory:\(serverName)") {
            serverRootDirectory.appendingPathComponent(serverName)
        }
    }

    static func serverModsDirectory(serverName: String) -> URL {
        cachedURL(key: "serverModsDirectory:\(serverName)") {
            serverDirectory(serverName: serverName).appendingPathComponent(AppConstants.DirectoryNames.mods)
        }
    }

    static func serverPluginsDirectory(serverName: String) -> URL {
        cachedURL(key: "serverPluginsDirectory:\(serverName)") {
            serverDirectory(serverName: serverName).appendingPathComponent(AppConstants.DirectoryNames.plugins)
        }
    }

    static func serverWorldDirectory(serverName: String) -> URL {
        cachedURL(key: "serverWorldDirectory:\(serverName)") {
            serverDirectory(serverName: serverName).appendingPathComponent("world")
        }
    }
    ...
    static func invalidatePaths(forServerName serverName: String) {
        let keys = [
            "serverDirectory:\(serverName)",
            "serverModsDirectory:\(serverName)",
            "serverPluginsDirectory:\(serverName)",
            "serverWorldDirectory:\(serverName)",
        ] as [NSString]
        cacheQueue.sync(flags: .barrier) {
            for key in keys {
                pathCache.removeObject(forKey: key)
