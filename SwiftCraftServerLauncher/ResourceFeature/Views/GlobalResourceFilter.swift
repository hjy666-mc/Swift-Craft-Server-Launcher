...
func filterCompatibleServers(
    ...
    let supportedLoaders = Set(detail.loaders.map { $0.lowercased() })
    let resourceTypeLowercased = resourceType.lowercased()
    return serverRepository.servers.filter { server in
        guard supportedVersions.contains(server.gameVersion) else {
            return false
        }
        switch (resourceTypeLowercased, server.serverType) {
        case ("plugin", .paper):
            return !supportedLoaders.isDisjoint(with: Set(["paper", "bukkit", "spigot", "purpur", "folia"]))
        case ("mod", .fabric):
            return supportedLoaders.contains("fabric")
        case ("mod", .forge):
            return supportedLoaders.contains("forge")
        default:
            return false
        }
    }
