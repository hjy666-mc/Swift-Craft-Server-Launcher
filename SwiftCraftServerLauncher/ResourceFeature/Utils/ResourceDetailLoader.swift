...
enum ResourceDetailLoader {
    ...
    static func loadProjectDetail(
        ...
        resourceType: String,
        serverRepository: ServerRepository? = nil
    ) async -> (
        detail: ModrinthProjectDetail,
        compatibleGames: [GameVersionInfo],
        compatibleServers: [ServerInstance]
    )? {
        _ = gameRepository
        ...
        let compatibleServers: [ServerInstance]
        if let serverRepository {
            compatibleServers = filterCompatibleServers(
                detail: detail,
        ...
        return (detail, compatibleGames, compatibleServers)
    }
